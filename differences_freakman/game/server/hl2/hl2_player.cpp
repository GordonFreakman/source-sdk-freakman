...
#ifdef HL2MP
	CSuitPowerDevice SuitDeviceSprint( bits_SUIT_DEVICE_SPRINT, 2.0f );				// 100 units in 4 seconds //25.0f
#else
	CSuitPowerDevice SuitDeviceSprint( bits_SUIT_DEVICE_SPRINT, 2.0f );				// 100 units in 8 seconds //12.5f
#endif

#ifdef HL2_EPISODIC
	CSuitPowerDevice SuitDeviceFlashlight( bits_SUIT_DEVICE_FLASHLIGHT, 0.001 );	// 100 units in 90 second 1.111
#else
	CSuitPowerDevice SuitDeviceFlashlight( bits_SUIT_DEVICE_FLASHLIGHT, 0.001 );	// 100 units in 45 second 2.222
#endif
...
void CHL2_Player::Precache( void )
{
	...
	PrecacheScriptSound( "Player.Freakman" );
	PrecacheScriptSound( "Player.Wing" );
}
...
void CHL2_Player::PreThink(void)
{
	...
	//Custom stuff
	if ( m_bHasLongJump == true ){// Hm.. might look on to this later...
		/*
		if (m_bHasWings == false){// Glitches happened with both enabled
		
			if (!((m_nButtons | m_afButtonPressed | m_afButtonReleased) & IN_JUMP))
				return;
			if ((m_afButtonPressed & IN_JUMP))
			{
				...
				UTIL_TraceHull(WorldSpaceCenter() - Vector(0, 0, 16), vecCheckDir, NAI_Hull::Mins(HULL_TINY_CENTERED), NAI_Hull::Maxs(HULL_TINY_CENTERED), MASK_PLAYERSOLID, this, COLLISION_GROUP_PLAYER, &tro);
				NDebugOverlay::Line( tro.startpos, tro.endpos, 0,255,0, true, 10 );
				if (tro.fraction == 1.0 && !tro.startsolid)
				{
					// Now trace down!
					UTIL_TraceLine(vecCheckDir, vecCheckDir - Vector(0, 0, 64), MASK_PLAYERSOLID, this, COLLISION_GROUP_NONE, &tro);
					if (tro.fraction == 1.0 && !tro.startsolid)
					{
						Vector vecNewVelocity = GetAbsVelocity();
						vecNewVelocity.z += 250;
						vecNewVelocity.y += 2000;
	...
	if (m_bHasWings == true){ //Kinda Simple, but would fuck up soon
		//if (!((m_nButtons | m_afButtonPressed | m_afButtonReleased) & IN_JUMP))
		//	return;
		if (m_afButtonPressed & IN_JUMP)
		{
			EmitSound("Player.Wing");
			Vector vecNewVelocity = GetAbsVelocity();
			vecNewVelocity.z += 250;
			SetAbsVelocity(vecNewVelocity);
		}
