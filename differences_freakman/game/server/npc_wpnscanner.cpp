...
void CNPC_WpnScanner::RunTask(const Task_t *pTask)
{
	...
	case TASK_WPNSCANNER_ATTACK:
	{
		...
		*/
		trace_t tr;
		UTIL_TraceLine(vecFirePos, vecFirePos + vecToTarget * MAX_COORD_INTEGER, MASK_SHOT, this, COLLISION_GROUP_NONE, &tr);
		NDebugOverlay::Line(vecFirePos + Vector(0, 0, -3), tr.endpos, 255, 255, 0, false, 0.1f);

		CBaseEntity *pObjectStruck = tr.m_pEnt;

		if (pObjectStruck->IsAlive())
		{
			CTakeDamageInfo damageInfo(this, this, 1.0f, DMG_GENERIC | DMG_DISSOLVE); //tr.m_pEnt->GetHealth()
