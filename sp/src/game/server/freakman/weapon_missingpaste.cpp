...
//
// Bug Bait Weapon
// Modded By Wirelex.exe lel

class CWeaponMissingPaste : public CBaseHLCombatWeapon
{
	DECLARE_CLASS(CWeaponMissingPaste, CBaseHLCombatWeapon);
public:

	DECLARE_SERVERCLASS();

	CWeaponMissingPaste(void);
...
IMPLEMENT_SERVERCLASS_ST(CWeaponMissingPaste, DT_WeaponMissingPaste)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS(weapon_missingpaste, CWeaponMissingPaste);
#ifndef HL2MP
PRECACHE_WEAPON_REGISTER(weapon_missingpaste);
#endif

BEGIN_DATADESC(CWeaponMissingPaste)
...
// Purpose: 
//-----------------------------------------------------------------------------
CWeaponMissingPaste::CWeaponMissingPaste(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMissingPaste::Spawn(void)
{
	BaseClass::Spawn();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMissingPaste::FallInit(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMissingPaste::Precache(void)
{
	BaseClass::Precache();

	UTIL_PrecacheOther("npc_grenade_missingpaste");

	PrecacheScriptSound("Weapon_MissingPaste.Splat");
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMissingPaste::Drop(const Vector &vecVelocity)
{
	...
	SetTouch(&CWeaponMissingPaste::BugbaitStickyTouch); //Change?
...
//-----------------------------------------------------------------------------
void CWeaponMissingPaste::BugbaitStickyTouch(CBaseEntity *pOther)
{
	if (!pOther->IsWorld())
		return;
...
// Input  : *pPicker - 
//-----------------------------------------------------------------------------
void CWeaponMissingPaste::OnPickedUp(CBaseCombatCharacter *pNewOwner)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMissingPaste::PrimaryAttack(void)
{
	if (m_bRedraw)
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMissingPaste::SecondaryAttack(void)
{
	// Squeeze!
	CPASAttenuationFilter filter(this);

	EmitSound(filter, entindex(), "Weapon_MissingPaste.Splat");

	if (CGrenadeMissingPaste::ActivateMissingPasteTargets(GetOwner(), GetAbsOrigin(), true) == false)
	{
...
// Input  : *pPlayer - 
//-----------------------------------------------------------------------------
void CWeaponMissingPaste::ThrowGrenade(CBasePlayer *pPlayer)
{
	...
	CGrenadeMissingPaste *pGrenade = MissingPasteGrenade_Create(vThrowPos, vec3_angle, vThrowVel, QAngle(600, random->RandomInt(-1200, 1200), 0), pPlayer);
...
//			*pOperator - 
//-----------------------------------------------------------------------------
void CWeaponMissingPaste::Operator_HandleAnimEvent(animevent_t *pEvent, CBaseCombatCharacter *pOperator)
{
	CBasePlayer *pOwner = ToBasePlayer(GetOwner());
	switch (pEvent->event)
	{
...
//-----------------------------------------------------------------------------
bool CWeaponMissingPaste::Reload(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMissingPaste::ItemPostFrame(void)
{
	CBasePlayer *pOwner = ToBasePlayer(GetOwner());
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponMissingPaste::Deploy(void)
{
	CBasePlayer *pOwner = ToBasePlayer(GetOwner());
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponMissingPaste::Holster(CBaseCombatWeapon *pSwitchingTo)
{
	m_bRedraw = false;
	m_bDrawBackFinished = false;
...
// Input  : true - 
//-----------------------------------------------------------------------------
void CWeaponMissingPaste::SetSporeEmitterState(bool state)
{
	m_bEmitSpores = state;
}
